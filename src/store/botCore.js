...
const handlers = {
  ...
  delay: async (senderId, payload, blockName) => {
    // delay handled in main function
  },
  list: async (senderId, payload, blockName) => {
    if (payload.elements?.length > 0) {
      await sendListTemplate(senderId, payload.elements, payload.buttons || []);
    }
  },
...
async function handleBlock(senderId, blockName) {
  ...
  let delay = 1000;
  if (type === "delay") {
    delay = Number(payload.time) || 1000;
  }

  if (next && type !== "input") {
    setTimeout(() => handleBlock(senderId, next), delay);
  }
}
